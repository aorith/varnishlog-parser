...
const (
	// TxTypeSession groups requests in the same session.
	TxTypeSession TxType = "Session"
	// TxTypeRequest is a client side request.
	TxTypeRequest TxType = "Request"
	// TxTypeBereq is a backend side request.
...
// RecordByTag returns the first or last record with the given tag.
...
func (t *Transaction) GetBackendConnStr() string {
	...
	record := r.(BackendOpenRecord) // nolint
...
func (t TransactionSet) Transactions() []*Transaction {
	...
	slices.SortFunc(txs, func(a, b *Transaction) int {
		if n := cmp.Compare(a.VXID, b.VXID); n != 0 {
			return n
		}
...
// GroupRelatedTransactions returns txs grouped by their parent tx.
func (t TransactionSet) GroupRelatedTransactions() [][]*Transaction {
	roots := t.UniqueRootParents(true)

	var txs [][]*Transaction // nolint:prealloc
...
func (t TransactionSet) UniqueRootParents(includeSession bool) []*Transaction {
	...
	slices.SortFunc(parentTxs, func(a, b *Transaction) int {
